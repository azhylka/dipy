...
def generate_tractogram_with_dirs(double[:,::1] seed_positions,
                                  double[:,::1] seed_directions,
                                  StoppingCriterion sc,
                                  TrackerParameters params,
                                  PmfGen pmf_gen,
                                  ...
                                  int nbr_threads=0,
                                  float buffer_frac=1.0):
    """Generate a tractogram recording per-step chosen vertex indices.

    Same as ``generate_tractogram`` but yields ``(track, vertex_indices)``
    tuples where ``vertex_indices`` is an int32 array of sphere vertex
    indices chosen by the propagator at each streamline step (-1 at the
    seed point).
    ...
    ------
    track : ndarray
        Streamline coordinates in world space, shape (N, 3).
    vertex_indices : ndarray
        Chosen sphere vertex index at each step, shape (N,), dtype int32.
    ...
    while seed_start < _len:
        ...
        length_arr = <int*> malloc(_plen * sizeof(int))
        status_arr = <StreamlineStatus*> malloc(_plen * sizeof(int))
        dirs_arr = <int**> malloc(_plen * sizeof(int*))

        if (streamlines_arr == NULL or length_arr == NULL
                or status_arr == NULL or dirs_arr == NULL):
            raise MemoryError("Memory allocation failed")

        generate_tractogram_c_with_dirs(
            seed_positions[seed_start:seed_end],
            seed_directions[seed_start:seed_end],
            nbr_threads, sc, params, pmf_gen,
            streamlines_arr, length_arr, status_arr, dirs_arr)
        ...
        for i in range(seed_end - seed_start):
            if ((status_arr[i] == VALIDSTREAMLIME or params.return_all)
                ...
                s = np.asarray(<cnp.float_t[:length_arr[i]*3]> streamlines_arr[i])
                track = s.copy().reshape((-1, 3))
                idx_s = np.asarray(<int[:length_arr[i]]> dirs_arr[i])
                idx_arr = idx_s.copy().astype(np.int32)
                yield np.dot(track, lin_T) + offset, idx_arr
            free(streamlines_arr[i])
            free(dirs_arr[i])
...
cdef void generate_tractogram_c_with_dirs(double[:,::1] seed_positions,
    ...
    Same as generate_tractogram_c but also populates ``dirs_arr`` with
    the sphere vertex index chosen by the propagator at each step.
    ...
    cdef:
        ...
        int* stream_idx
        int* chosen_dirs
        int dirs_buf_size = params.max_nbr_pts * 2 + 1
    ...
    for i in prange(_len, nogil=True, num_threads=nbr_threads, schedule='dynamic', chunksize=64):
        stream = <double*> malloc((params.max_nbr_pts * 3 * 2 + 1) * sizeof(double))
        stream_idx = <int*> malloc(2 * sizeof(int))
        chosen_dirs = <int*> malloc(dirs_buf_size * sizeof(int))
        memset(chosen_dirs, -1, dirs_buf_size * sizeof(int))

        status[i] = generate_local_streamline_with_dirs(
            &seed_positions[i][0],
            &seed_directions[i][0],
            stream,
            stream_idx,
        ...
        # copy streamline points
        lengths[i] = stream_idx[1] - stream_idx[0] + 1
        streamlines[i] = <double*> malloc(lengths[i] * 3 * sizeof(double))
        memcpy(&streamlines[i][0], &stream[stream_idx[0] * 3], lengths[i] * 3 * sizeof(double))

        # copy chosen direction indices
        dirs_arr[i] = <int*> malloc(lengths[i] * sizeof(int))
        memcpy(&dirs_arr[i][0], &chosen_dirs[stream_idx[0]], lengths[i] * sizeof(int))
...
cdef StreamlineStatus generate_local_streamline_with_dirs(
                                                ...
                                                TrackerParameters params,
                                                PmfGen pmf_gen) noexcept nogil:
    """Generate a streamline from a seed, recording chosen vertex indices.

    Same as generate_local_streamline but also writes the chosen sphere
    vertex index (from stream_data[99]) at each tracking step into
    ``chosen_dirs``.  The buffer layout mirrors ``stream``:
    ``chosen_dirs[k]`` corresponds to ``stream[k*3 : k*3+3]``.
    ...
    stream : double*
        Buffer to store the generated streamline.
    stream_idx : int*
        Buffer to store the start/end indices of the streamline.
    chosen_dirs : int*
        Output buffer (size >= max_nbr_pts*2+1) for chosen vertex indices.
        -1 indicates no direction was chosen (seed point or unused slot).
    sc : StoppingCriterion
        Stopping criterion for the streamline.
    params : TrackerParameters
        Parameters for the streamline generation.
    ...
    fast_numpy.copy_point(seed, &stream[params.max_nbr_pts * 3])
    stream_idx[0] = stream_idx[1] = params.max_nbr_pts
    chosen_dirs[params.max_nbr_pts] = -1  # seed point: no chosen direction
    ...
    for i in range(1, params.max_nbr_pts):
        if params.tracker(&point[0], &voxdir[0], params, stream_data, pmf_gen, &rng) == TrackerStatus.FAIL:
            break
        chosen_dirs[params.max_nbr_pts + i] = <int>stream_data[99]
        # update position
        for j in range(3):
            point[j] += voxdir[j] * params.inv_voxel_size[j] * params.step_size
        fast_numpy.copy_point(point, &stream[(params.max_nbr_pts + i )* 3])
