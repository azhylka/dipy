...
def get_streamlines_mask(streamlines, affine, mask_volume):
    mask_volume = np.array(mask_volume, dtype=np.uint32, copy=True)
    lin_T, offset = _mapping_to_voxel(affine)
    mask = _streamlines_mask(streamlines, mask_volume, lin_T, offset)
    return mask
...
@cython.wraparound(False)
@cython.cdivision(True)
cdef cnp.uint32_t[:,:,:] _streamlines_mask(list streamlines,
                         cnp.uint32_t[:,:,:] mask,
                         lin_T, offset):
    """Filters streamlines based on whether or not they pass through a ROI,
    using a line-based algorithm for compressed streamlines.
    ...
    for streamline_idx in range(nb_streamlines):
        # Can't call _to_voxel_coordinates because it casts to int
        streamline = np.dot(streamlines[streamline_idx], lin_T) + offset
